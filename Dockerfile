...
RUN curl --fail --location --retry 3 --header "Cookie: oraclelicense=accept-securebackup-cookie; " \
  ...
  -o /tmp/jdk-8u102-linux-x64.tar.gz \
  && mkdir -p /opt/java \
  && tar -zvxf /tmp/jdk-8u102-linux-x64.tar.gz -C /opt/java/ \
  && rm -rf /tmp/jdk-8u102-linux-x64.tar.gz ${JAVA_HOME}/src.zip ${JAVA_HOME}/javafx-src.zip
